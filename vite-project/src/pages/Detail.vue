<template>
    <van-nav-bar class="white"
        left-arrow fixed @click="$router.go(-1)"
    />
<div class="content" v-if="data.detail">
  <div class="header clear">
    <h2><img :src="data.detail.auth_icon" alt=""/></h2><p>{{data.detail.auth}}</p></div>
  <div class="cont">
    <h3>{{data.title}}</h3>
    <div class="time"><p>{{moment(data.time)}} <span><img src="/assets/img/zan.png" alt=""/></span></p></div>
    <div class="text-box" v-html="data.detail.content">
    </div>
  </div>
</div>
...
<script setup>
  ...
  const {apiName,id}=route.query
  const data=ref({})
  onMounted(async()=>{
    const res=await queryDetail({id,apiName})
      // res.err非空校验
